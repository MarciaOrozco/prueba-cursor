...
const nutricionistaSchemas = {
  buscarNutricionistas: {
    query: Joi.object({
      ...
      modalidad: Joi.array().items(
        Joi.string().valid('presencial', 'virtual', 'mixta')
      ).optional(),
...
const turnoSchemas = {
  ...
  reprogramarTurno: {
    ...
    body: Joi.object({
      fecha: Joi.date().iso().greater('now').required(),
      hora: Joi.string().pattern(/^([01]?[0-9]|2[0-3]):[0-5][0-9]$/).required(),
      modalidad: Joi.string().valid('presencial', 'virtual', 'mixta').optional()
    })
  }
};
...
const pacienteSchemas = {
  obtenerTurnosPaciente: {
    ...
    query: Joi.object({
      estado: Joi.array().items(
        Joi.string().valid('pendiente', 'confirmado', 'cancelado', 'completado', 'reprogramado')
      ).optional(),
      limit: Joi.number().integer().min(1).max(100).default(20).optional(),
      offset: Joi.number().integer().min(0).default(0).optional()
    })
...
const responseSchemas = {
  ...
  perfilNutricionista: Joi.object({
    ...
    totalResenas: Joi.number().integer().min(0).required(),
    foto: Joi.string().uri().optional(),
    formacion: Joi.array().items(Joi.object({
    ...
    horariosAtencion: Joi.array().items(Joi.object({
    ...
    reseñas: Joi.array().items(Joi.object({
      id: Joi.string().uuid().required(),
  ...
  turno: Joi.object({
    ...
    hora: Joi.string().pattern(/^([01]?[0-9]|2[0-3]):[0-5][0-9]$/).required(),
    modalidad: Joi.string().valid('presencial', 'virtual', 'mixta').required(),
    estado: Joi.string().valid('pendiente', 'confirmado', 'cancelado', 'completado', 'reprogramado').required(),
    metodoPago: Joi.string().valid(
      'efectivo',
      'transferencia',
      'tarjeta_credito',
